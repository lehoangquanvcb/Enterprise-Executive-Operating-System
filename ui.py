...
def inject_css() -> None:
    ...
    .info {{border-left-color:{COLORS['info']};}}
    .section-label {{color:{COLORS['muted']};font-size:.78rem;text-transform:uppercase;
...
def executive_card(topic, finding, action, owner, sla, severity="warning") -> None:
    st.markdown(
        f'<div class="exec-card {severity}"><b>{topic}</b> — {finding}<br>'
...
def download_df(df, label: str, filename: str) -> None:
    st.download_button(
        label,
...
def guarded_section(title: str, fn) -> None:
    try:
        fn()
    except Exception as exc:
        st.warning(f"{title} is temporarily unavailable: {exc}")
